import { describe, expect, it } from 'vitest';
import { stripExifFromJpeg } from '../../src/lib/engines/exif';

describe('stripExifFromJpeg', () => {
  it('returns non-JPEG blobs unchanged', async () => {
    const pngBlob = new Blob([new Uint8Array([0x89, 0x50, 0x4e, 0x47])], { type: 'image/png' });
    const result = await stripExifFromJpeg(pngBlob);
    expect(result).toBe(pngBlob);
  });

  it('returns a JPEG blob when given a valid JPEG', async () => {
    const validJpegBytes = new Uint8Array([
      ,
    ]);
    const jpegBlob = new Blob([validJpegBytes], { type: 'image/jpeg' });
    const stripped = await stripExifFromJpeg(jpegBlob);
    expect(stripped.type).toBe('image/jpeg');
  });
});
