...
export async function tiffToJpg(file: File): Promise<Blob> {
...
async function imageDataToPngBlob(data: ImageData, width: number, height: number): Promise<Blob> {
  ...
  if (!ctx) throw new Error('tiff-to-jpg: 2D context unavailable');
  ctx.putImageData(data, 0, 0);
  return new Promise<Blob>((resolve, reject) => {
    canvas.toBlob((b) => {
      if (b) resolve(b);
      else reject(new Error('tiff-to-jpg: canvas.toBlob returned null'));
    }, 'image/png');
  });
}
